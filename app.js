(function () {
  ...
  var WEATHER_URL = 'https://api.open-meteo.com/v1/forecast?latitude=49.2032&longitude=16.5943&current=temperature_2m,weather_code,is_day&hourly=temperature_2m,weather_code,is_day&forecast_days=2&timezone=Europe%2FPrague';
  ...
  var el = {
    ...
    weatherCompare: document.getElementById('weather-compare'),
    weatherCompareNowIcon: document.getElementById('weather-compare-now-icon'),
    weatherCompareNowTemp: document.getElementById('weather-compare-now-temp'),
    weatherCompareEndIcon: document.getElementById('weather-compare-end-icon'),
    weatherCompareEndTemp: document.getElementById('weather-compare-end-temp'),
    weatherCompareEndLabel: document.getElementById('weather-compare-end-label'),
  ...
  function renderWeather(data, fromCache) {
    ...
    el.weatherDesc.innerHTML = weatherDescription(data.code) + '<span>Brno</span>';
    ...
    if (el.overlayWeatherDesc) el.overlayWeatherDesc.innerHTML = weatherDescription(data.code) + '<span>Brno</span>';
    if (el.overlayWeatherIcon) el.overlayWeatherIcon.innerHTML = weatherSymbol(data.code, data.isDay !== 0);
    if (el.weatherCompareNowTemp) el.weatherCompareNowTemp.textContent = Math.round(data.temperature) + ' °C';
    if (el.weatherCompareNowIcon) el.weatherCompareNowIcon.innerHTML = weatherSymbol(data.code, data.isDay !== 0);
  ...
  function parseHourlyForecast(json) {
    if (!json || !json.hourly || !json.hourly.time) return null;
    var times = json.hourly.time || [];
    var temperatures = json.hourly.temperature_2m || [];
    var codes = json.hourly.weather_code || [];
    var isDays = json.hourly.is_day || [];
    var rows = [];
    var i;
    for (i = 0; i < times.length; i++) {
      var ms = Date.parse(times[i]);
      var temperature = Number(temperatures[i]);
      var code = Number(codes[i]);
      var isDay = Number(isDays[i]);
      if (!isNaN(ms) && isFinite(temperature) && isFinite(code)) {
        rows.push({ timeMs: ms, temperature: temperature, code: code, isDay: isDay });
      }
    }
  ...
  function forecastForTime(targetMs) {
    var rows = state.weatherHourly;
    if (!rows || !rows.length || !isFinite(targetMs)) return null;
    var best = rows[0];
    var bestDistance = Math.abs(best.timeMs - targetMs);
    var i;
    for (i = 1; i < rows.length; i++) {
      var distance = Math.abs(rows[i].timeMs - targetMs);
  ...
  function renderWeatherComparison(ctx, now) {
    if (!el.weatherCompare) return;
    if (!ctx || ctx.kind !== 'active' || !state.weatherCurrent || !state.weatherHourly) {
      el.weatherCompare.className = 'weather-compare hidden';
      return;
    }
    ...
    if (el.weatherCompareEndTemp) el.weatherCompareEndTemp.textContent = Math.round(forecast.temperature) + ' °C';
    if (el.weatherCompareEndIcon) el.weatherCompareEndIcon.innerHTML = weatherSymbol(forecast.code, forecast.isDay !== 0);
    if (el.weatherCompareEndLabel) el.weatherCompareEndLabel.textContent = 'v ' + ctx.endLabel;
    el.weatherCompare.className = 'weather-compare';
  }

  function hidePublicWeather() {
    if (document.body) document.body.classList.add('weather-unavailable');
    if (el.weatherCompare) el.weatherCompare.className = 'weather-compare hidden';
  }
  ...
  function parseWeatherResponse(json) {
    ...
    return {
      current: { temperature: temperature, code: code, isDay: isDay },
      hourly: parseHourlyForecast(json)
    };
  }
  ...
  function updateWeather() {
    ...
    requestWeather(url)
      .then(function (json) {
        var parsed = parseWeatherResponse(json);
        state.weatherHourly = parsed.hourly;
        state.lastWeatherUpdate = new Date().getTime();
        if (state.weatherRetryTimer) {
          clearTimeout(state.weatherRetryTimer);
          state.weatherRetryTimer = null;
        }
        renderWeather(parsed.current, false);
        showPublicWeather();
        tick();
      })
  ...
  function tick() {
    ...
    render(now, ctx);
    renderWeatherComparison(ctx, now);
