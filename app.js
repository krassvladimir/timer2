...
function updateDepartureWeatherIcon(code, isDay) {
  var el = document.querySelector('.weather-end-icon');
  if (!el) return;
  var icon = '☁';
  if (code === 0) icon = isDay ? '☀' : '☾';
  else if (code === 1 || code === 2) icon = isDay ? '⛅' : '☁';
  else if (code === 3) icon = '☁';
  else if (code === 45 || code === 48) icon = '≋';
  else if ((code >= 51 && code <= 67) || (code >= 80 && code <= 82)) icon = '☂';
...
(function () {
  ...
  function adjustedNowMs() {
    // v6.2.13: odpočet vždy vychází přímo z aktuálního systémového času tabletu.
  ...
  function updateEndWeather(now, ctx) {
    ...
    el.weatherEnd.innerHTML = '<span>↗</span> ' + Math.round(forecast.temperature) + ' °C' + rain;
  }
...
document.addEventListener('DOMContentLoaded', function () {
  var end = document.querySelector('.weather-end');
  if (!end) return;
  var obs = new MutationObserver(function () {
    updateDepartureWeatherIcon(2, true);
  });
  obs.observe(end, { childList: true, subtree: true, characterData: true });
  updateDepartureWeatherIcon(2, true);
});
