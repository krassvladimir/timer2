(function () {
  ...
  function renderWeather(data, fromCache) {
    ...
    el.weatherDesc.textContent = 'Brno · ' + weatherDescription(data.code);
    el.weatherIcon.textContent = weatherSymbol(data.code, data.isDay !== 0);
    if (el.overlayWeatherTemp) el.overlayWeatherTemp.textContent = Math.round(data.temperature) + ' °C';
    if (el.overlayWeatherDesc) el.overlayWeatherDesc.textContent = 'Brno · ' + weatherDescription(data.code);
    if (el.overlayWeatherIcon) el.overlayWeatherIcon.textContent = weatherSymbol(data.code, data.isDay !== 0);
    state.weatherStatus = (fromCache ? 'uloženo' : 'online') + ' · ' + Math.round(data.temperature) + ' °C';
    if (el.diagWeather) el.diagWeather.textContent = state.weatherStatus;
  }

  function loadCachedWeather() {
    try {
      var raw = localStorage.getItem('justyou_weather');
      if (!raw) return;
      var parsed = JSON.parse(raw);
      if (parsed && parsed.data) renderWeather(parsed.data, true);
    } catch (e) {}
  }

  function updateWeather() {
    if (!navigator.onLine || !window.fetch) {
      state.weatherStatus = 'offline';
      if (el.diagWeather) el.diagWeather.textContent = state.weatherStatus;
      loadCachedWeather();
      return;
    }
    ...
    fetch(WEATHER_URL, { cache: 'no-store' })
      ...
      .then(function (json) {
        ...
        var data = {
          ...
          isDay: Number(json.current.is_day)
        };
        state.lastWeatherUpdate = new Date().getTime();
        renderWeather(data, false);
        try {
          localStorage.setItem('justyou_weather', JSON.stringify({
            savedAt: state.lastWeatherUpdate,
            data: data
          }));
        } catch (e) {}
      })
      .catch(function () {
        state.weatherStatus = 'nedostupné';
        if (el.diagWeather) el.diagWeather.textContent = state.weatherStatus;
        loadCachedWeather();
      });
  }
    ...
    try {
      ...
      for (r = 0; r < repeats; r++) {
        ...
        var partials = [
          ...
          { ratio: 2.01, gain: 0.44, decay: 0.76 },
          { ratio: 2.72, gain: 0.22, decay: 0.58 },
          { ratio: 4.08, gain: 0.10, decay: 0.42 }
        ];
  ...
  function playWarning(kind) {
    if (kind === 'warn10') {
      playGong({ repeats: 1, base: 440, duration: 1.45, level: 1.0 });
    } else if (kind === 'warn5') {
      playGong({ repeats: 2, base: 440, duration: 1.35, gap: 0.55, level: 1.0 });
    } else if (kind === 'warn1') {
      playGong({ repeats: 3, base: 392, duration: 1.28, gap: 0.42, level: 1.0 });
    } else if (kind === 'last10') {
      playTick();
    } else if (kind === 'ended') {
      playGong({ repeats: 1, base: 294, duration: 2.45, level: 1.0 });
    }
  }
