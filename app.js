(function () {
  ...
  var INTERNET_SYNC_INTERVAL = 15 * 60 * 1000;
  var APP_UPDATE_INTERVAL = 60 * 60 * 1000;
  ...
  var WEATHER_URL = 'https://api.open-meteo.com/v1/forecast?latitude=49.210211&longitude=16.602327&current=temperature_2m,weather_code,is_day&hourly=temperature_2m,weather_code,is_day&forecast_days=2&timezone=Europe%2FPrague';
  ...
  var el = {
    ...
    endWeather: document.getElementById('end-weather'),
    endWeatherIcon: document.getElementById('end-weather-icon'),
    endWeatherTemp: document.getElementById('end-weather-temp'),
    endWeatherDesc: document.getElementById('end-weather-desc'),
  ...
  function adjustedNowMs() {
    // Čas se počítá ze systémového času tabletu a tiché síťové korekce.
    // Korekce nic nezobrazuje na veřejné obrazovce a přežije i výpadek internetu.
    return Date.now() + state.clockOffsetMs;
  }

  function loadStoredClockOffset() {
    try {
      var savedOffset = Number(localStorage.getItem('justyou_clock_offset_ms'));
      var savedAt = Number(localStorage.getItem('justyou_clock_offset_at'));
      if (isFinite(savedOffset) && Math.abs(savedOffset) < 5 * 60 * 1000 &&
          isFinite(savedAt) && Date.now() - savedAt < 7 * 24 * 60 * 60 * 1000) {
        state.clockOffsetMs = savedOffset;
      }
    } catch (e) {}
  }

  function saveClockOffset(offset, syncedAt) {
    try {
      localStorage.setItem('justyou_clock_offset_ms', String(offset));
      localStorage.setItem('justyou_clock_offset_at', String(syncedAt));
    } catch (e) {}
  }
    ...
    xhr.onreadystatechange = function () {
      ...
      if (xhr.status >= 200 && xhr.status < 400 && !isNaN(serverMs)) {
        ...
        if (Math.abs(newOffset) < 5 * 60 * 1000) {
          ...
          state.internetSyncStatus = 'OK';
          saveClockOffset(newOffset, finished);
  ...
  function renderWeather(data, fromCache) {
    ...
    el.weatherDesc.innerHTML = weatherDescription(data.code) + '<span>Hrnčířská 11</span>';
    ...
    if (el.overlayWeatherDesc) el.overlayWeatherDesc.innerHTML = weatherDescription(data.code) + '<span>Hrnčířská 11</span>';
    if (el.overlayWeatherIcon) el.overlayWeatherIcon.innerHTML = weatherSymbol(data.code, data.isDay !== 0);
    state.weatherStatus = (fromCache ? 'uloženo' : 'online') + ' · Hrnčířská 11 · ' + weatherDescription(data.code) + ' · ' + Math.round(data.temperature) + ' °C';
  ...
  function parseWeatherResponse(json) {
    ...
    var hourly = null;
    if (json.hourly && Array.isArray(json.hourly.time) && Array.isArray(json.hourly.temperature_2m) && Array.isArray(json.hourly.weather_code)) {
      hourly = {
        time: json.hourly.time,
        temperature: json.hourly.temperature_2m,
        code: json.hourly.weather_code,
        isDay: Array.isArray(json.hourly.is_day) ? json.hourly.is_day : []
      };
    }
    return { temperature: temperature, code: code, isDay: isDay, hourly: hourly };
  }

  function forecastForEnd(now, ctx) {
    var hourly = state.weatherHourly;
    if (!hourly || !ctx || ctx.kind !== 'active' || !hourly.time || !hourly.time.length) return null;

    var end = new Date(now.getTime() + Math.max(0, ctx.remaining) * 1000);
    var bestIndex = -1;
    ...
    for (i = 0; i < hourly.time.length; i++) {
      ...
      var diff = Math.abs(t - end.getTime());
      if (diff < bestDiff) { bestDiff = diff; bestIndex = i; }
    }
    if (bestIndex < 0) return null;

    var temperature = Number(hourly.temperature[bestIndex]);
    var code = Number(hourly.code[bestIndex]);
    var isDay = Number(hourly.isDay[bestIndex]);
    if (!isFinite(temperature) || !isFinite(code)) return null;
    return { temperature: temperature, code: code, isDay: isDay };
  }

  function renderEndWeather(now, ctx) {
    if (!el.endWeather) return;
    if (!ctx || ctx.kind !== 'active' || ctx.remaining <= 600) {
      el.endWeather.className = 'end-weather hidden';
      return;
    }
    var forecast = forecastForEnd(now, ctx);
    if (!forecast) {
      el.endWeather.className = 'end-weather hidden';
      return;
    }
    el.endWeather.className = 'end-weather';
    if (el.endWeatherTemp) el.endWeatherTemp.textContent = Math.round(forecast.temperature) + ' °C';
    if (el.endWeatherDesc) el.endWeatherDesc.textContent = weatherDescription(forecast.code);
    if (el.endWeatherIcon) el.endWeatherIcon.innerHTML = weatherSymbol(forecast.code, forecast.isDay !== 0);
  }
  ...
  function render(now, ctx) {
    ...
    if (ctx.kind === 'night') {
      ...
      if (el.endWeather) el.endWeather.className = 'end-weather hidden';
    ...
    // Ukazatel zbývajícího času: na začátku je plný a s odpočtem se zkracuje.
    var pct = Math.max(0, Math.min(100, (ctx.remaining / SLOT_SECONDS) * 100));
  ...
  function runWatchdog() {
    ...
    if (state.lastTickRawTime && nowMs - state.lastTickRawTime > 20000) {
  ...
  window.addEventListener('online', function () { syncInternetTime(); updateWeather(); checkForAppUpdate(); tick(); }, false);
  ...
  loadStoredClockOffset();
  tick();
  updateWeather();
  syncInternetTime();
  setInterval(tick, 1000);
  setInterval(syncInternetTime, INTERNET_SYNC_INTERVAL);
  setInterval(checkForAppUpdate, APP_UPDATE_INTERVAL);
