(function () {
  ...
  function adjustedNowMs() {
    // v6.2.7: odpočet vždy vychází přímo z aktuálního systémového času tabletu.
  ...
  function updateEndWeather(now, ctx) {
    ...
    if (!ctx || ctx.kind !== 'active' || !state.hourlyWeather) {
    ...
    var rain = forecast.precipitation === null ? '' : ' · déšť ' + Math.round(forecast.precipitation) + ' %';
    el.weatherEnd.className = 'weather-end';
    el.weatherEnd.innerHTML = '<span>V ' + ctx.endLabel + '</span> ' + Math.round(forecast.temperature) + ' °C' + rain;
  }
