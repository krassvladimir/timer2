...
(function () {
  ...
  var el = {
    ...
    weatherEndIcon: document.getElementById('weather-end-icon'),
  ...
  function adjustedNowMs() {
    // v6.2.16: odpočet vždy vychází přímo z aktuálního systémového času tabletu.
  ...
  function updateEndWeather(now, ctx) {
    ...
    if (!ctx || ctx.kind !== 'active' || !state.hourlyWeather || ctx.remaining <= 10 * 60) {
      el.weatherEnd.className = 'weather-temp weather-end hidden';
      el.weatherEnd.innerHTML = '';
      if (el.weatherEndIcon) el.weatherEndIcon.innerHTML = '';
      return;
    }
    ...
    el.weatherEnd.className = 'weather-temp weather-end';
    el.weatherEnd.innerHTML = Math.round(forecast.temperature) + ' °C';
    if (el.weatherEndIcon) {
      el.weatherEndIcon.innerHTML = weatherSymbol(forecast.code, forecast.isDay !== 0);
    }
  }
